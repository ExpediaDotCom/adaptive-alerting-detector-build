...
from ._constant_threshold import constant_threshold
...
def detector(detector_config):
    ...
    """
    __detector_type = detector_config['type'] if 'type' in detector_config else 'constant-threshold'
    __detector = None
    # TODO: add detector schema validation
    # Add elif for additional detector types
    if detector_config['type'] == "constant-threshold":
        __detector=constant_threshold(**detector_config)
    else:
        raise AdaptiveAlertingDetectorBuildError(f"Unknown detector type '{__detector_type}'")
    return __detector
