...
class TestCTHighwatermarkDetector:
    def test_train_detector_for_cgp_metrics(self):
        detector_config = dict(
            hyperparams=dict(upper_weak_multiplier=1.05, upper_strong_multiplier=1.10, 
                hampel_window_size=10, hampel_n_sigma=3),
            detectorMetaData=dict(subtype="ct-highwatermark-detector")
        )
        ...
        detector_class.train(data, "LATENCY_AVG")
        print("detector model data after training:\n", related.to_json(detector_class))
        assert isclose(detector_class.config.params.thresholds.upper_strong_threshold, 43711.030462, rel_tol=0.0001)
        assert isclose(detector_class.config.params.thresholds.upper_weak_threshold, 41724.165441, rel_tol=0.0001)
    ...
    def test_train_detector_for_elb_metrics(self):
        ...
        detector_class.train(data, "LATENCY_AVG")
        print("detector model data after training:\n", related.to_json(detector_class))
        assert isclose(detector_class.config.params.thresholds.upper_strong_threshold, 0.612911, rel_tol=0.0001)
        assert isclose(detector_class.config.params.thresholds.upper_weak_threshold, 0.585052, rel_tol=0.0001)
