from profile.stationarity_checker import DEFAULT_MAX_ADF_PVALUE, DEFAULT_SIGNIFICANCE
from .stationarity_types import StationarityResult, AnnotatedStationarityResult, AdfResultWrapper


def annotate_stationarity(stationarity_result: StationarityResult, timeseries_name: str = 'timeseries',
                          significance: str = DEFAULT_SIGNIFICANCE, max_adf_pvalue=DEFAULT_MAX_ADF_PVALUE) -> AnnotatedStationarityResult:
    """
    Takes the given StationarityResult, generates some descriptive text fields and returns it as an
    AnnotatedStationarityResult.
    ...
    :return: AnnotatedStationarityResult
    """
    ...
    return AnnotatedStationarityResult(adf_result, adf_summary, p_value_display, stationarity_display, is_stationary,
                                       test_stat_display)


def _stationarity_details(is_stationary):
    return f"{'IS' if is_stationary else 'is NOT'} stationary (we {'can' if is_stationary else 'cannot'} " \
           f"reject the Null Hypothesis that there is a unit root).\n" \
           f"{'At least one of the tests PASSED' if is_stationary else 'None of the tests PASSED'} "
...
def _test_stat_details(adf_result_wrapper: AdfResultWrapper, significance: str):
    ...
    return f"{'PASSED' if pass_test_stat_test else 'FAILED'}: " \
...
def _p_value_details(p_value: float, max_adf_pvalue: float):
    ...
    return f"{'PASSED' if pass_p_value_test else 'FAILED'}: " \
