...
def test_calculate_sigma_for_integers():
    ...
    assert isclose(builders.calculate_sigma(sample), 5.1130086, rel_tol=0.0001)

def test_calculate_sigma_for_floats():
    sample = [35.2, 42.9, 37.8, 9.3, 15.0, 10.1, 20]
    assert isclose(builders.calculate_sigma(sample), 14.015671, rel_tol=0.0001)

def test_calculate_sigma_raises_error_for_single_value():
    sample = [35.2]
    with pytest.raises(Exception) as exception:
        assert isclose(builders.calculate_sigma(sample), 0)
    assert str(exception.value) == "Sample must have at least two elements"

def test_calculate_sigma_raises_error_with_empty_list():
    with pytest.raises(Exception) as exception:
        assert isclose(builders.calculate_sigma([]), 0)
    assert str(exception.value) == "Sample must have at least two elements"

def test_calculate_mean_for_floats():
    sample = [35.2, 42.9, 37.8, 9.3, 15.0, 10.1, 20]
    assert isclose(builders.calculate_mean(sample), 24.328571, rel_tol=0.0001)

def test_calculate_mean_for_single_value():
    sample = [35.2]
    assert isclose(builders.calculate_mean(sample), 35.2, rel_tol=0.0001)

def test_calculate_sigma_thresholds():
    sigma = 5
    mean = 10
    multiplier = 3
    upper, lower = builders.calculate_sigma_thresholds(sigma, mean, multiplier)
...
def test_calculate_quartiles():
    ...
    q1, median, q3 = builders.calculate_quartiles(sample)
    assert q1 == 6.5
    assert median == 12.5
    assert q3 == 19.0
...
def test_calculate_quartile_thresholds():
    q1, q3 = 2, 5
    multiplier = 1.5
    upper, lower = builders.calculate_quartile_thresholds(q1, q3, multiplier)
